...
**更新时间**: 2026-05-25
**代码版本**: v2.2.0（本地 tag，待推送 GitHub Release）
**最新发布**: v2.0.1（含 DMG / EXE）
...
### v2.0.1（2026-05-22）— 样式修复补丁

| 变更 | 详情 |
|------|------|
| 全站配色统一 | 移除彩虹色角色/类别标签，改用 shadcn/Linear 中性 secondary 语言 |
| Sidebar 缩进修复 | 移除 `pl-12` 多余左缩进 |
| Sidebar footer | 角色权限标签改为纯文字，去掉有色 badge |
| 错误记录 | 分类标签全改 muted，仅崩溃保留 destructive |
| KDS / 订单 / 生产单 | 状态 badge 去掉 blue/emerald，action icon 改 muted-foreground |
| 库存 / 支出 / 供应商 | 交易类型 / 费用类型 / 渠道 badge 统一 secondary |
...
## ✅ v2.2.0 — 多端接入與桌位自助（代碼完成 2026-05-23，待 GitHub Release）
...
## 📋 v2.3.0 — 打印传播力（Kano Must-Do，规划中）

> **目标**：把餐單 / 收據 / 廚房單做成「會被拍照、會被轉發、會產生二次傳播」的載體，而不是只承擔信息傳遞。

| 功能 | 说明 | 复杂度 |
|------|------|--------|
| 今日灵感卡 | 打印结果可附加可复制的短句，作为社群分享文案素材 | 低 |
| 运势 / 抽签模块 | 仅保留 `小吉 / 中吉 / 大吉` 三档，做成轻量可选附加内容 | 低 |
| 点阵感艺术风格 | 收据/订单预览支持 BBS 质感、点阵感、可爱插画位与可爱图层 | 中 |
| Romance / ins 風文案 | 预置浪漫诗句、外文 ins 感短句模板，增强拍照传播性 | 低 |
| 品牌化打印模板 | 为厨房单、收据、标签预留统一的创意插槽与品牌资产位 | 中 |
