...
export function MarketingPage() {
  ...
  // Load default receipt template elements for read-only preview
  useEffect(() => {
    invoke<{ id: number; content: string; is_active: boolean }[]>(
      "get_print_templates", { templateType: "receipt" }
    ).then(templates => {
      const arr = Array.isArray(templates) ? templates : [];
      const def = arr.find(t => t.is_active) ?? arr[0];
      if (def) {
        try {
          const parsed = JSON.parse(def.content);
          if (Array.isArray(parsed.elements) && parsed.elements.length > 0) {
            setReceiptElements(parsed.elements);
          }
        } catch { /* keep defaults */ }
      }
    }).catch(() => {});
